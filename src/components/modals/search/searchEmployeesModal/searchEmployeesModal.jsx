...
import employeesApiService from './searchEmployeesService'
import CloseButton from '../../../reusableComponents/CloseButton'
import SearchModalEmployeesSearchBox from './searchModalEmployeesSearchBox'
import ReloadItems from '../../../reusableComponents/reloadItems'
import LoadingIcon from '../../../reusableComponents/loadingIcon'

const SearchEmployeesModal = ({ closeModal, showModal, searchSelection, position}) => {
   ...
   const loadSearches = queryParameters => {
    ...
    //     searchEmployeePosition: position,
    //     searchEmployeeCompany: logic.getCompany().id
    // })
    const searchObj = {
        q: {
            active_eq: true,
            employee_jobs_job_name_eq: position || null,
            employee_jobs_company_id_eq: logic.companyId.id || null,
            sorts: 'person_last_name asc',
        }
    }
    if (queryParameters && queryParameters.searchName) searchObj.q.person_first_name_cont = queryParameters.searchName
    if (queryParameters && queryParameters.searchSurName) searchObj.q.person_last_name_cont = queryParameters.searchSurName

    employeesApiService.getEmployees(searchObj)
        .then(data => {
            setTotalItems(data.total)
            const subs = data.json.employees
            setIsLoading(false)
            setItems(subs)
        })
        .catch(err => {
    ...
    return <div id="searchSubzonesModal" className={showHideClassName} tabIndex="-1" draggable-modal="true">
            ...
            <div className="modal-content">
                ...
                <div className="modal-header">
                    ...
                    <h3 className="smaller lighter blue no-margin">
                        {t('search')} {t('lowercase', { text: t('employees') })}
                    </h3>
                </div>
                <div className="modal-body">
                    <div className="row">
                        <ReloadItems doReload={handleRefreshClick}></ReloadItems>
                        <SearchModalEmployeesSearchBox requestSearch={handleRequestSearch}></SearchModalEmployeesSearchBox>
                    </div>
                            ...
                            <table className="table table-hover-search table-striped">
                                    ...
                                    <tr className="ng-table-sort-header">
                                        <th title="" className="header  sortable sort-asc">
                                            <div className=" ng-scope" >
                                                <span className="ng-binding sort-indicator">{t('name')}</span>
                                            </div>
                                        </th>
                                ...
                                <tbody>
                                    {items && items.map(((item, index) =>
                                        <tr onClick={e => handleSelectItemClick(e, item)} key={index} ng-repeat="fleet in $data track by $index" className="pointer">
                                            <td data-title="tablesTranslations.fleet" data-sortable="'name'">
                                                {`${item && item.person && item.person.first_name} ${item && item.person && item.person.last_name}`}
                                            </td>
