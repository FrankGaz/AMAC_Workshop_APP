...
import unitiesModalApiService from './unitiesService'
import LoadingIcon from '../../../reusableComponents/loadingIcon'
import CloseButton from '../../../reusableComponents/CloseButton'
import SearchModalUnitiesSearchBox from './searchModalUnitiesSearchBox'
import ReloadItems from '../../../reusableComponents/reloadItems'

const SearchUnitiesModal = ({ closeModal, showModal, searchSelection, zoneId }) => {
    ...
    // flag that controls whether there is an api call going on or not
    ...
    const loadSearches = queryParameters => {
        ...
        const query = {
            ...queryParameters,
            searchZoneIdId: zoneId
        }
        logic.configureQueryParameters(query)

        showModal && unitiesModalApiService.getUnities()
            .then(data => {
                setTotalItems(data.total)
                setItems(data.json.unities)
                setIsLoading(false)
            })
            .catch(err => {
                setIsLoading(false)
    ...
    return <div id="searchUnitiesModal" className={showHideClassName} tabIndex="-1" draggable-modal="true">
            ...
            <div className="modal-content">
                ...
                <div className="modal-header">
                    ...
                    <h3 className="smaller lighter blue no-margin">
                        {t('search')} {t('lowercase', { text: t('unity') })}
                    </h3>
                </div>
                ...
                <div className="modal-body">
                    ...
                    <div className="row">
                        ...
                        <SearchModalUnitiesSearchBox requestSearch={handleRequestSearch} />
                    </div>
                    ...
                    <div className="row table-responsive-row">
                        <div className="col-md-12 table-responsive table-search">
                            <table className="table table-hover-search table-striped">
                                ...
                                <tbody data-test-id='unityDetailsHeader'>
                                    {items && items.map(((item, index) =>
                                        <tr data-test-id='unityDetails' onClick={e => handleSelectItemClick(e, item)} key={index} ng-repeat="fleet in $data track by $index" className="pointer">
                                            <td data-test-id="unityDetails.name" data-sortable="'name'">
                                                {item.name}
                                            </td>
...
export default SearchUnitiesModal